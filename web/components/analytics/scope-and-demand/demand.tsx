...
return <div className="space-y-3 rounded-[10px] border border-custom-border-200 p-3">
    <h5 className="text-xs text-red-500">{t("components.analytics.demand")}</h5>
    <div>
        <h4 className="text-custom-text-100 text-base font-medium">{t("components.analytics.total-open-tasks")}</h4>
        ...
        {defaultAnalytics?.open_issues_classified.map((group) => {
            ...
            return (
                ...
                />
                            ...
                            <span
                                className="ml-1 rounded-3xl bg-custom-background-80 px-2 py-0.5 text-[0.65rem] text-custom-text-200">
                  {group.state_count}
                </span>
                        </div>
    ...
    <div
        className="!mt-6 flex w-min items-center gap-2 whitespace-nowrap rounded-md border border-custom-border-200 bg-custom-background-80 p-2 text-xs">
        <p className="flex items-center gap-1 text-custom-text-200">
            <PlayIcon className="h-4 w-4 -rotate-90" aria-hidden="true"/>
            <span>{t("components.analytics.estimate-demand")}</span>
        </p>
