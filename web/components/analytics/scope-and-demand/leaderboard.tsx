...
export const AnalyticsLeaderboard: React.FC<Props> = ({
  ...
  return <div className="p-3 border border-custom-border-200 rounded-[10px]">
              ...
              >
                <div className="flex items-center gap-2">
                  {user && user.avatar && user.avatar !== "" ? (
                      ...
                      <div
                          className="grid place-items-center flex-shrink-0 rounded-full bg-gray-700 text-[11px] capitalize text-white h-4 w-4">
                        {user.display_name !== "" ? user?.display_name?.[0] : "?"}
                      </div>
                  )}
                  <span className="break-words text-custom-text-200">
                {user.display_name !== "" ? `${user.display_name}` : t("components.analytics.no-assignee")}
              </span>
                </div>
